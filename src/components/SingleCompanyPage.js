...
const SingleCompanyPage = () => {
    ...
    let title = '';
    if(data !== null && dataCompanies.length > 0){
        title = dataCompanies[0].name
        document.title = `${title} - Statistics and Charts | StreamCamel`;
    }
    
    let description = '';
    if(data !== null && dataCompanies.length>0 && dataCompanies[0].description !== null) {
        description = utils.textToParagraphs(dataCompanies[0].description);
    }

    let iconurl = '';
    if (data !== null && dataCompanies.length > 0 && dataCompanies[0].url !== null) {
